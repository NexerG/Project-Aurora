...
namespace ArctisAurora.EngineWork.Renderer
{
    ...
    internal unsafe class Swapchain
    {
        ...
        internal void CreateImageView(ref ImageView _iv, ref Image _im, ImageAspectFlags _aspect, Format _f)
        {
            ...
            if (VulkanRenderer._vulkan!.CreateImageView(VulkanRenderer._logicalDevice, ref _createInfo, null, out _iv) != Result.Success)
            {
        ...
        internal void CreateShadowmapRenderPass()
        {
            ...
            fixed (AttachmentDescription* _attachmentPtr = _attachments)
            {
                ...
                if (Rasterizer._vulkan.CreateRenderPass(Rasterizer._logicalDevice, ref _renderPassInfo, null, out _shadowmapRenderPass) != Result.Success)
                {
                    throw new Exception("failed to create render pass!");
                }
            }
