...
- [x] **UI Engine landing 6a — the base gap (2026-09-06)** — landed, See `ClaudeMemory/Decisions/ui-engine-stack.md`
	- [ ] **landings 6b–6d: 11 of the 56 subclasses are across** — panel, stack, button, icon, label, title bar, window frame, splitter, split view, scrollable, scroll thumb. Left: tabs (needs the drag's drop/hint/ghost half), menus and the browser set (need the context-menu hooks back), then 6c text/documents and 6d the delete. Per-landing detail in `ClaudeMemory/Context/ui-engine-plan.md`
