...
- [ ] UI
	...
	- [ ] **animation system — nothing can change over time on its own (plan agreed 2026-09-18).** One procedural core for UI and game on its own thread; palettes and gradients merge into one paint word and become its data; springs take interaction and physics impulses. See `ClaudeMemory/Context/animation-plan.md`
		- [ ] **`StopAll` scans every binding, and `Control.OnDestroy` calls it** — destroying 20k buttons stalled 2.27 s, which fits O(subtree × bindings). Likely cause, not isolated; `bindings` never shrinks → `animation-core` § Measured at scale
		- [ ] **animation writes its own data, `Main.Apply` deleted (deferred 2026-09-24)** — setter-path properties become pool fields, dirty marking and `onDone` go through drained pools; fork 3 (button state in the shader or `Emit`) open → `animation-in-place-plan`
...
- [ ] **frame scheduler — every system a step of one graph, every core used (plan agreed 2026-09-23).** Steps 1–3 landed (graph, access check, `Threads`/`MaxFps`; Main as 5 steps, edge steps, lanes deleted, animation writes in place; `Jobs.For` over Animation's rows); next: step 4 SIMD tracks. See `ClaudeMemory/Context/frame-scheduler-plan.md`
	...
	- [ ] **`Anim.Step` 1.4–1.8 ms at 20k, not under 1 ms** — the parallel pass is 0.4–0.7 ms; the serial `AnimationValues` append is the rest and leaves with `Main.Apply` → `frame-scheduler` § Step 3
