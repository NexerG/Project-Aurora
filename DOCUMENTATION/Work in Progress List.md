...
- [ ] UI
	- [x] texture table — sampler array is indexed by *texture* (`ControlData.textureIndex`) instead of by instance (`gl_InstanceIndex`), so the descriptor cap is "distinct textures" (256) rather than "controls" (was 50,000). Every glyph in a font shares one slot, and control churn no longer writes a descriptor at all — only a pool capacity change rebuilds. Also cleared the 44-byte `ControlData` stride alignment validation error (now 48). Phase D's "texture set" arriving early. See `DOCUMENTATION/ClaudeMemory/Decisions/glyphs-as-pool-data.md`. **Runs clean with validation on; NOT yet eyeballed**
		...
		- [x] edge + outline colour (2026-08-19) — two strokes, because there are two distance fields to stroke. `EdgeThickness`/`EdgeColorHex` band the rounded-box SDF that `CornerRadius` already produces, so it is a border on the control's rectangle in design-space pixels; `OutlineWidth`/`OutlineColorHex` are a second threshold on the mask's MSDF distance, so they trace the letter rather than the letter's box, in screen pixels. The edge contributes its **own** coverage instead of multiplying into the mask — otherwise `EdgeThickness` on a container drew nothing, since containers are exactly the controls masked `invisible` — while the outline multiplies as the fill does. Both branch on `> 0.0`: a zero-width stroke evaluated as a stroke half-tints the antialiased boundary pixel of every control in the tree. Edge composites last so it wins over an outline underneath. `ControlData` 68 → 100 bytes, checked on both sides (`Unsafe.SizeOf` == 100 and `spirv-dis` `ArrayStride 100`, offsets identical) rather than assumed. Hex only — no `ControlColor`-style enum twins. **NOT GUI-verified**: nothing authors either yet, so the render is unchanged by construction. See `ClaudeMemory/Decisions/control-edge-and-outline.md`
			- [ ] `UITypeSchema.xsd`/`SchemaManifest.xml` regenerate on the next run, so the four properties are missing from the committed schema until then. Authoring works regardless — nothing validates at load
