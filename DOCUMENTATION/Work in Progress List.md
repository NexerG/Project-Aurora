...
- [x] **UI Engine landing 6a — the base gap (2026-09-06)** — landed, See `ClaudeMemory/Decisions/ui-engine-stack.md`
	- [ ] **landings 6b–6d: 6b, 6c, 6c2, 6d's delete and prefix drop are built (2026-09-15)** — the old stack is deleted and `Core.UISystem` is gone; Thorium, Carbon and a blank Editor boot on the new one. Left: by-hand checks of the name and confirm prompts and tear-off, and the two collapses → `ui-engine-plan` § 6d
	- [ ] **an armed key capture binds the next mouse press** — `InputHandler.FirstPressed` counts mouse buttons, so clicking away from a listening keybind row rebinds it; clicking Settings' close bound `ExitApplication` to `MouseLeft`. Both stacks → `ui-engine-plan` § 6c2
	- [ ] **seen once: a block's `StylingType` missing from a saved note** — after a long mixed chain (select-all bold → undo → block styling → armed colour → typed character → drag-select → px size) every span change saved and the block styling did not. Not reproduced; the same restyle saved correctly twice in isolation → `ui-engine-stack` § landing 6c
	- [ ] **word and line selection on the new stack is NOT GUI-verified** — double and triple click never reach `tapCount` 2 under synthetic input, which 6b's own tab-caption rename proves is the driver and not the stack; needs a by-hand check → `ui-engine-plan` § 6c-b
	...
	- [ ] **6b3's splits are not GUI-verified** — Tab ▸ Split, View ▸ Split, Ctrl+\ / Ctrl+Shift+\ and Close on an inactive tab all need two tabs in one view, which only a drag makes → `context-menus`
	...
	- [ ] **the drag reaches its claimant; the drop half is still missing** — a left press on a control whose new `draggable` flag is set calls `StartDrag()` from the base `OnPointerPress`, publishing the `Dragging` context and telling the parent `ChildDraggedOut`; `UIEngine.CheckDrag` then runs each tick from `Poll`, holding **one drag target the way one control is hovered** and bubbling `DraggingOverStart` / `DraggingOver` / `DraggingOverEnd` from it; a left release calls `UIEngine.EndDrag(point)` **ahead of the release guards** (a click only counts where it started, a drag ends wherever the pointer got to), which fires `DraggingOverEnd`, then `Control.FinishDrag(dragged, point)` on the target, then clears both. `HitTest` gained a `skip` parameter, rejected at the subtree root, so the dragged control *and everything under it* stay out of the answer — the old stack skips nothing and only survives because `TabViewControl` happens to be an ancestor of the button being dragged. Landed since: per-tick delivery (`onDrag`/`onDragStop`/`SolveDrag`), the stale-release guard, and `Poll`'s `ownsDrag` exemption with `WindowOf` — `WindowFrameControl`, `SplitterControl` and `ScrollThumbControl` all run on them. Missing and load-bearing: **the drop does not walk up**, where `OfferDrop` offered each ancestor in turn; and `EndDrag` clears `Dragging` *after* the callbacks. Seven controls claim a drag in the old stack (`TabStripButtonControl`, `ScrollThumbControl`, `SplitterControl`, `DocumentEditorControl`, `TextBoxControl`, `WindowFrameControl`'s grips, Carbon's `SpanChartControl` twice) and **five need only per-tick delivery**, which now exists — `ScrollThumbControl`, `SplitterControl` and `WindowFrameControl`'s grips are ported and running on it; `DocumentEditorControl`'s selection drag landed at 6c and `TextBoxControl` has no port of its own yet. The drop, hint and ghost half (`HitFor`, `WindowAt`, `OfferDrop`, `UpdateDropHint`, the `Hinted` context, `RaiseHovered`, `DragGhost`, `draggingOpacity`) exists for tab dragging alone: `TabViewControl` is the repo's only `ResolveDrop`/`ResolveDropHint` implementor and `TabStripButtonControl` its only `DragGhost.Show` caller. Full table in `ClaudeMemory/Decisions/ui-engine-stack.md` § the drag gap
	- [ ] **context menu gaps** — no monitor clamping or flipping, no Escape or keyboard, a windowed panel renders 1:1 under autoscale, an in-window open or close re-lays out the whole window → `context-menus`
	...
	- [ ] **a root destroyed before its first layout leaks its lazy children** — `ResolveLayout` still lays out a dead root left in `_dirtyRoots`, so grips and scroll thumbs are built under dead parents; UIElements then skips resequence every frame. `ProfileScenario` dodges it → `engine-profiling` §15
...
- [x] **a swap is a decoration action, and the engine ships the settings shell (2026-08-29)** — landed, See `ClaudeMemory/Decisions/ui-document-registry.md`
	- [ ] **no pool-count evidence for a swap.** The round trip is "no FATAL and the app stayed alive", not the `1325/1325` check the landing pass used; a `--profile-pools` capture now records the occupancy per frame, but no swap has been run against one → `engine-profiling` §14
...
- [ ] renderer update
	- [ ] separate whole renderer features away from modules like `TimelineSemaphores` into an array. (settings) — `RenderingModule` exposes `features` and `features12` but **no `features13`**, so no module can request a 1.3 feature; `PreInitialize` hardcodes `DynamicRendering` → `ui-engine-clip-as-coverage`
	- [ ] resource manager
	- [ ] Buffer device address (BDA) for vertex buffers — `CreateBuffer` no longer chains `AddressBitKhr` onto every allocation (2026-09-12); this now needs the flag *and* `bufferDeviceAddress` requested → `mapped-streaming-buffers` § 7
	- [ ] descriptor sets
