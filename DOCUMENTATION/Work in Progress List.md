...
- [x] **UI Engine landing 6a — the base gap (2026-09-06)** — landed, See `ClaudeMemory/Decisions/ui-engine-stack.md`
	...
	- [ ] **a `TextRunControl`'s colour does not reach its glyphs after the first measure** — `BuildRuns` reads `colorHex` at measure and the setter invalidates arrange only, so a label recoloured later keeps the old ink; the 6c format bar calls `InvalidateLayout` itself. Latent, not observed elsewhere → `ui-engine-stack` § landing 6c
