...
- [ ] UI
	...
	- [ ] **animation system — nothing can change over time on its own (plan agreed 2026-09-18).** One procedural core for UI and game on its own thread; palettes and gradients merge into one paint word and become its data; springs take interaction and physics impulses. See `ClaudeMemory/Context/animation-plan.md`
		- [ ] **in-place animation is NOT GUI-verified (landed 2026-09-26)** — scenario and boot only. Unchecked: theme crossfade, press colours, state bindings, menu slide, file-tree collapse incl. one interrupted by a re-expand (`onDone` now a frame later) → `animation-in-place-plan` § Verification
		- [ ] **the five animation pools never shrink (2026-09-25)** — no edge step in `Frame.frame.xml`, so no `FrameEdge`; `Animations` rows are indexed by track id and never freed, so its count is the high-water mark (memory only since the awake list) → `pool-shrink` § Known gaps
...
- [ ] **frame scheduler — every system a step of one graph, every core used (plan agreed 2026-09-23).** Steps 1–3 landed (graph, access check, `Threads`/`MaxFps`; Main as 5 steps, edge steps, lanes deleted, animation writes in place; `Jobs.For` over Animation's rows); step 4 (SIMD tracks): 4.0 measured, ~0.1 ms left to win, rest undecided. See `ClaudeMemory/Context/frame-scheduler-plan.md`
	- [ ] **scheduler overhead ~20 µs a frame** — up from 4 µs with 7 stages; unpinned stages wake parked workers every frame → `frame-scheduler` § Step 2 Measured
	- [ ] **22 profiling batches lost at shutdown** — `Profiling.Flush` times out on parked workers' last batch (warns "not handed within 2000ms"); seen since step 1 → `frame-scheduler` § Step 2 Known gaps
	- [ ] **render redraws unpaced when Main has nothing new** — ~3,500 fps against Main's 4 at 200k; burns a core and floods the Render capture lane (275k frames per 64 MB file) → `animation-core` § Measured at 200k
