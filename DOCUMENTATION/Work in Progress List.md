...
- [x] **UI Engine landing 6a — the base gap (2026-09-06)** — landed, See `ClaudeMemory/Decisions/ui-engine-stack.md`
	- [ ] **landings 6b–6d are agreed and unbuilt** — port the 56 subclasses (`Next`-prefixed) and their support, then the text and document stack, then delete `Core.UISystem` and drop every prefix. Full per-landing detail and verification in `ClaudeMemory/Context/ui-engine-plan.md`
	...
	- [ ] **the drag gesture runs end to end, but never reaches its claimant** — a left press on a control whose new `draggable` flag is set calls `StartDrag()` from the base `OnPointerPress`, publishing the `NextDragging` context and telling the parent `ChildDraggedOut`; `UIEngine.CheckDrag` then runs each tick from `Poll`, holding **one drag target the way one control is hovered** and bubbling `DraggingOverStart` / `DraggingOver` / `DraggingOverEnd` from it; a left release calls `UIEngine.EndDrag(point)` **ahead of the release guards** (a click only counts where it started, a drag ends wherever the pointer got to), which fires `DraggingOverEnd`, then `Control.FinishDrag(dragged, point)` on the target, then clears both. `HitTest` gained a `skip` parameter, rejected at the subtree root, so the dragged control *and everything under it* stay out of the answer — the old stack skips nothing and only survives because `TabViewControl` happens to be an ancestor of the button being dragged. Missing and load-bearing: **the claimant is told nothing** — no per-tick position, no end-of-gesture callback; **a release outside the window leaves the drag live for good**, since `Poll` returns early and nothing re-checks the button (the `ownsDrag` exemption and the stale-release guard are one fix); **the drop does not walk up**, where `OfferDrop` offered each ancestor in turn; and `EndDrag` clears `NextDragging` *after* the callbacks. Seven controls claim a drag in the old stack (`TabStripButtonControl`, `ScrollThumbControl`, `SplitterControl`, `DocumentEditorControl`, `TextBoxControl`, `WindowFrameControl`'s grips, Carbon's `SpanChartControl` twice) and **five need only per-tick delivery** — `onDrag`/`ResolveDrag`, `SolveDrag` with its stale-release guard, `StopDrag`, the release branch, and `Poll`'s `ownsDrag` exemption with `WindowOf`. The drop, hint and ghost half (`HitFor`, `WindowAt`, `OfferDrop`, `UpdateDropHint`, the `NextHinted` context, `RaiseHovered`, `DragGhost`, `draggingOpacity`) exists for tab dragging alone: `TabViewControl` is the repo's only `ResolveDrop`/`ResolveDropHint` implementor and `TabStripButtonControl` its only `DragGhost.Show` caller. Full table in `ClaudeMemory/Decisions/ui-engine-stack.md` § the drag gap
	- [ ] **nothing on the new stack opens a context menu** — `contextMenus`/`BuildContextMenu`/`OpenContextMenu` were built at 6a and removed; they return at 6b with the controls that host them. `ContextMenus` and `pressSwallowed` stay in the old stack, and `ContextMenu="…"` in a `*.ui.xml` now binds to nothing and is silently skipped — seven live sites across `UI.ui.xml` and `Workspace.ui.xml`
...
- [ ] **context menus get redone, and the factory goes first** (user, 2026-08-28) — `ContextMenus.menuFactory` is a `Func<ContextMenuControl>` a host swaps at startup, which `Thorium.Main` does to get `WindowedContextMenuControl`. It is the **only** factory in the solution — the other two `Func<>` fields are a predicate and a sort provider — and it contradicts the standing "no factory pattern in this codebase" decision in `ClaudeMemory/Context/multi-windowing-plan.md`. The whole system wants revisiting, not just the swap point
