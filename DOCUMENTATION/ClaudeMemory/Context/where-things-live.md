...
| a quad's colour on the GPU — paint words, the paint table, when a control's colour is resolved | `UI.VulkanControl` (`paint`, `alpha`, `edgePaint`); `UI.UIEngine.Collect` → `UI.Control.InheritPaint` (every drawn control, every frame); `UI.Palettes.Paints` (`GpuPaint`, pool `Paints`); `UI.Gradients.Pool` (pool `Gradients`); `Rendering.Modules.UIEngineModule.TableMirror`; `Shaders/UIEngine/UIEngine.vert` `resolvePaint` | `AuroraEngine/Data/XML/Documents/Pools.pools.xml` | [[ui-palettes]] |
...
| clipping on the **new** stack, a child cut at its parent's edge, overflow, `ClipToBounds` (on by default) | `UI.Control` (`arrange.clip`, `clipOutOfBounds`, constructor), `UIEngine.Collect`; `Shaders/UIEngine/UIEngine.frag` (`inClip`) | — | [[ui-clip-by-default]], [[ui-engine-clip-as-coverage]] |
...
| data pools (the ECS rework); finding a typed column, `GetRef`/`GetSpan` cost, column lookup | `Data.DataManager`, `DataPool` (`GetRef`, `GetSpan`, `_byKey`), `ColumnKey<T>`, `ColumnKeys`, `PoolColumn`, `TransformData` | `AuroraEngine/Data/XML/Documents/Pools.pools.xml` | [[ecs-rework-data-pools]], [[cross-system-change-notification]] |
| pool growth, shrinking a pool, capacity, wasted memory, stable id reuse | `Data.DataPool` (`TryShrink`, `Resize`, `ShrinkAfterSeconds`, `_freeIds`, `_versionFloor`); `Render.Modules.UIEngineModule.MirrorTooBig` | `Pools.pools.xml` (`Growth`, `GrowthValue`) | [[pool-shrink]] |
...
| tick order, threads, job graph, stages, workers, which system runs when, frame rate cap, thread count, who may write a pool, frame steps, frame edges, pinned steps, main tick, parallel for, split a loop across threads, chunks | `Threading.FrameScheduler`, `FrameStep`, `Jobs` (`For`, `Chunk`, `IJobFor`), `ThreadingSettings`, `ThreadedSystem` (`RunStep`, `EndFrame`), `MainSystem` (actions `Main.Input`/`Logic`/`Apply`/`Layout`/`DrawLists`), `RenderSystem`, `PhysicsSystem`; `Animation.AnimationSystem`; `Data.DataPool.AssertAccess`; `Core.Engine` (`Input`, `Interpolate`) | `AuroraEngine/Data/XML/Documents/Frame.frame.xml` | [[frame-scheduler]], [[animation-core]] |
| animation, tweens, easing, springs, animating a property, animating a pool field in place, animating alpha or a border, sleeping / waking animations, which tracks get stepped, an animation re-running layout, when onDone runs, a track's row layout, hot / cold track data | `Animation.Animations` (`Tween`, `Spring`, `Retarget`, `Stop`, `Wake`, `Awake`, `DrainDone`), `AnimationSystem` (`Execute`, `Emit`), `AnimationTrack` (hot), `TweenParams`, `KeyParams`, `SpringParams`, `TrackCold`, `Curve`/`EaseKind`, `Spring`, `A_Animatable` (column, field, `LayoutChange`), `AnimatableProperty`, `DirtyLayout`, `FinishedTrack`; `UI.UIEngine.DrainLayoutDirty`; `Data.DataPool.ElementBytes`; `UI.Control.visual` (the `UIElements` `VulkanControl` column), `UI.Control.PaintRow` | `AuroraEngine/Data/XML/Documents/Pools.pools.xml` (`Animations`, `LayoutDirty`, `AnimationDone`, `UIElements`) | [[animation-core]] |
...
| SIMD, vectorising a math-heavy loop, SoA / AoS / AoSoA, data layout, `Vector256` batches | — (guidance; no engine type follows it yet) | — | [../Patterns/simd-data-layout.md](../Patterns/simd-data-layout.md) |
...
| ticking, `OnTick`, which entities tick, the tick list, opting in or out of ticking, conditional ticking | `ECS.EngineEntity.Entity` (`SetTicking`, `tickSlot`, `HooksOf`), `Registry.EntityRegistry` (`ProcessTicks`, `SetTicking`) | — | [[entity-tick-group]] |
...
| profiling data pools — items, capacity, memory per frame; `--profile-pools` | `Diag.Profiling.Frame.Pool` (every pool at frame end, from `Threading.FrameScheduler.Run`), `Data.DataPool.ReservedBytes`, `Diag.ProfilingCaptureSetting.pools`; `Diag.FrameSpool.WriteFrame` (`<P>`) | `*/Data/XML/Settings/` (`<Profiling><ProfilingCapture Pools="true"/>`) | [[engine-profiling]] §14 |
...
| profiling animation, animation stress test, animation at scale; `--profile-scenario=animation` | `Diag.ProfileScenario` (`RunAnimation`, `BuildGrid`, `Ramp`); zones/counters `Anim.*` in `Animation.AnimationSystem`, `Animation.Animations` | engine `Animations/UI.anim.xml` (`profile-state`, `profile-margin`) | [[engine-profiling]] §17, [[animation-core]] § Measured at scale |
...
| the **new** measure/arrange pass, dirty roots, per-window ranges | `UI.UIEngine` (`ResolveLayout`, `RefreshWindowRanges`, `ElementOrder`, `NextControlOrder`); `UI.Control` (`Measure`, `Arrange`, `MeasureCore`, `ArrangeCore`, `InvalidateLayout`) | `Pools.pools.xml` (`SortAction`) | [[ui-engine-stack]] |
| data-oriented layout, the tree as row ranges, subtree ranges, layout kinds, parent row, a 1M-control UI | `UI.LayoutEngine` (`BuildStructure`, `VerifyStructure`, kind per type); `UI.LayoutNode`, `UI.LayoutNodeKind`; `UI.Control.node`; `UI.StackPanelControl` (`orientation`/`Spacing` mirrored) | `Pools.pools.xml` (`UIElements` + `LayoutNode`); `Frame.frame.xml` (`UIElements` edge before `Main.Layout`) | [layout-dod-plan](layout-dod-plan.md) |
