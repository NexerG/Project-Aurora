...
| animation, tweens, easing, springs, animating a property, animating a pool field in place, animation values | `Animation.Animations` (`Tween`, `Spring`, `Retarget`, `Stop`, `Write`, `ApplyValues`), `AnimationSystem`, `Curve`/`EaseKind`, `Spring`, `A_Animatable` (column, field, changed), `AnimatableProperty`, `AnimationValue`; `Data.DataPool.ElementBytes` | `AuroraEngine/Data/XML/Documents/Pools.pools.xml` (`Animations`, `AnimationValues`) | [[animation-core]] |
