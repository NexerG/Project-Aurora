...
| profiling — timing a tick phase, counting calls | `Diag.Profiling` (`Zone.Start`/`End`/`Increment`, `Report`); zones in `Core.Engine.MainTick`, `Threading.RenderSystem`; layout zones in `UI.UIEngine.ResolveLayout`, `UI.DocumentControl`, `UI.TextRunControl`, `UI.DocumentEditorControl` | — | [[engine-profiling]] |
