...
| keyframes, clips, animation XML, hover/press animation on a control, reversing an animation, state bindings (rest/hover/press values) | `Animation.AnimationLibrary` (`LoadAnimations`, `Clip`, `Binding`, `Sample`, `LocalTime`, `ParseValue`), `ClipDefinition`/`ClipTrackDefinition`/`KeyDefinition`, `BindingDefinition`/`BindingTrackDefinition`, `Keyframe`, `ClipLoop`, `Animations.Play`/`Direct`, `StateBinding`, `AnimationSystem` (keyframe driver); `UI.Control` — `clip`, `hoverClip`, `pressClip`, `stateBinding` | `*/Data/XML/Documents/Animations/*.anim.xml`; `Pools.pools.xml` (`Keyframes`); `*.ui.xml` attrs `Clip`, `HoverClip`, `PressClip`, `StateBinding` | [[animation-core]] |
