...
| gradients; a gradient that follows the palette, role stops, shade | `UI.Gradients` (`IndexOf`, `Word`), `UI.GpuGradientStop`; `UI.Palettes.RoleOffsets`; `UI.Control.gradient`, `gradientId`; `Shaders/UIEngine/UIEngine.frag` `stopColor` | `Thorium/Data/XML/Documents/Gradients.gradients.xml` | [[ui-gradients]] |
