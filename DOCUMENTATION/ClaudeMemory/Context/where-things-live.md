...
| delivering a drag to its **claimant**, the drag preview, context menus | **nothing on the new stack** — the target hears everything, the dragged control nothing. They return at 6b with the controls that use them; the old stack's are `UI.UICollisionHandling`, `UI.ContextMenus`, `UI.DragGhost` | — | [[ui-engine-stack]] |
