...
| tick order, threads, job graph, stages, workers, which system runs when, frame rate cap, thread count, who may write a pool | `Threading.FrameScheduler`, `FrameStep`, `ThreadingSettings`, `ThreadedSystem`, `MainSystem`, `RenderSystem`, `PhysicsSystem`; `Animation.AnimationSystem`; `Data.DataPool.AssertAccess`; `Core.Engine` | `AuroraEngine/Data/XML/Documents/Frame.frame.xml` | [[frame-scheduler]], [[animation-core]] |
