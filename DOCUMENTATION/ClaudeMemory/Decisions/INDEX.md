...
| [[next-context-menus]] | a context menu is a `*.menu.xml` a control names; entries bubble up until a control stops them; it is the root's last child when it fits and its own window when not | `NextContextMenus`, `NextContextMenuControl`, `ContextMenu`, `ContextMenuAsset`, `Control.contextMenu`, `NextMenuButtonControl`, `TabActions` |
| [[ui-engine-stack]] | **PARTIAL** — the UI is rebuilt in a new namespace beside the old one; `Control` is CPU, `VulkanControl` is one GPU quad. Landings 1–5 and 6a built: it draws, measures, arranges, takes a per-window slice, dispatches the pointer and the wheel, renders a styled paragraph as one control with a row per glyph, samples images, icons, masks and gradients, and builds a tree from XML. the drag gesture runs press to release against whatever it is over, but tells its claimant nothing; context menus are [[next-context-menus]] | `UIEngine`, `Control`, `ControlXml`, `ContainerControl`, `WindowRoot`, `TextRunControl`, `NextCaretControl`, `ArrangeData`, `ControlGeometry`, `VulkanControl`, `CornerRadii`, `PointerEvent`, `UIEngineModule` |
