...
| [[ui-draw-list]] | the GPU list is walked out of the tree each frame and culled to the screen, not stored in a pool | `DrawList`, `Control.Emit`, `UIEngine.BuildDrawLists`, `TextRunControl.Emit`, `UIEngineModule` |
| [[ui-draw-list-publish]] | the walk's cursor is separate from the count the render thread reads, published once at the end — the fix for the UI flicker, which was `Clear()` zeroing the count under the reader | `DrawList` (`Clear`, `Next`, `Publish`, `Count`), `UIEngine.BuildDrawLists`, `UIEngineModule.MirrorDrawList` |
