...
| Note | Settles | Key symbols |
|---|---|---|
| [[frame-scheduler]] | **PARTIAL** (step 1 of `frame-scheduler-plan`) — systems are steps of one frame graph in `Frame.frame.xml`; stages computed from per-column reads/writes, a loop is a load error; pools have no owner, a DEBUG check throws on an undeclared column; Render is `Dedicated`; `Threads` (1 = graph on one core) and `MaxFps` (0 = uncapped) settings; scheduler overhead ~4 µs a frame | `FrameScheduler`, `FrameStep`, `ThreadingSettings`, `ThreadedSystem.Step`, `DataPool.AssertAccess`, `DataManager.FrameEdge()` |
| [[animation-core]] | **PARTIAL** (slice 3 of `animation-plan`) — animation is a fourth `ThreadedSystem` (since 2026-09-23 a step of the frame graph — [[frame-scheduler]]); Main posts requests, Animation posts values back, applied at Main's `Drain`; ids and bindings live on Main with a generation; `FrameEdge` runs per owning system; `CommandOp.Post` messages a system, not a column; springs can follow named signals; `*.anim.xml` clips are keyframe tracks over a `Keyframes` pool, held hover/press clips reverse by `Direct`, state bindings are a spring on a 0/1/2 state; measured: 682 starts and ~1,024 values a tick per lane, ~0.34 µs a track, `StopAll` O(bindings) | `AnimationSystem`, `Animations` (`Play`, `Direct`), `AnimationLibrary`, `StateBinding`, `Signals`, `Curve`, `Spring`, `A_Animatable`, `ThreadedSystem.Post`/`OnPost`, `DataManager.FrameEdge()` |
