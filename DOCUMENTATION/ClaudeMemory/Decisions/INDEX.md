...
| [[animation-core]] | **PARTIAL** (slice 3 of `animation-plan`) — animation is a system stepped by the frame graph ([[frame-scheduler]]); Main writes requests into the animation pools directly (`Animations.Write`); values go through pool `AnimationValues`, applied by `Main.Apply`; pool-stored properties (`[A_Animatable(column, field, changed)]`) are written in place by `Animation.Step` and only invalidated on Main; ids and bindings live on Main with a generation; springs can follow named signals; `*.anim.xml` clips are keyframe tracks over a `Keyframes` pool, held hover/press clips reverse by `Direct`, state bindings are a spring on a 0/1/2 state; no start/value ceilings since 2026-09-23; ~0.34 µs a track, `StopAll` O(bindings) | `AnimationSystem`, `Animations` (`Play`, `Direct`, `Write`, `ApplyValues`), `AnimationLibrary`, `StateBinding`, `Signals`, `Curve`, `Spring`, `A_Animatable`, `AnimatableProperty`, `AnimationValue` |
