...
| [[ui-effects]] | effects are GPU tweens from a shared named table (like gradients); the row carries an index and a start time, the vertex shader eases offset/scale/rotation/alpha against engine time; per-glyph stagger at emit; starts when assigned; visual only | `Effects`, `GpuEffect`, `VulkanControl.effect`/`effectStart`, `Control.RestartEffect`, `StyleSpan.effect`, `Run.effect`, `UIEngine.vert` `ease` |
...
| [[ui-draw-list]] | the GPU list is walked out of the tree each frame and culled to the screen; a control under 12 on either axis draws without its children; **storage reversed** by [[ui-quads-pool]] — the walk and cull stand | `Control.Emit`, `UIEngine.BuildDrawLists`, `UIEngine.detailCullSize`, `TextRunControl.Emit`, `UIEngineModule` |
| [[ui-draw-list-publish]] | the walk's cursor is separate from the count the render thread reads, published once at the end — the fix for the UI flicker, which was `Clear()` zeroing the count under the reader; `DrawList` gone, the split lives on as the pool count vs the module's published range | `UIEngine.BuildDrawLists`, `UIEngineModule.PublishQuadRange`, `UIEngineModule.MirrorDrawList` |
| [[ui-quads-pool]] | the draw list is one handle-less `UIQuads` pool shared by every window, a published range each; geometry built at emit, `visual` copied; additive growth | `UIEngine.Quads`, `DataPool.Rewind`/`Append`, `Control.Emit(z)`, `UIEngineModule.PublishQuadRange`, `Pools.pools.xml` |
| [[ui-clip-by-default]] | a control clips its children to its bounds unless it sets `ClipToBounds="false"`; the flag is set in the constructor | `Control`, `Control.clipOutOfBounds`, `Control.WriteArranged`, `ArrangeFlags.Clip` |
| [[ui-engine-clip-as-coverage]] | the new stack's clip multiplies alpha instead of discarding, so the shader declares no `DemoteToHelperInvocation` capability; the old stack still discards | `UIEngine.frag`, `UIEngineModule`, `Renderer.PreInitialize` |
| [[context-menus]] | a context menu is a `*.menu.xml` a control names; entries bubble up until a control stops them; it is the root's last child when it fits and its own window when not | `ContextMenus`, `ContextMenuControl`, `ContextMenu`, `ContextMenuAsset`, `Control.contextMenu`, `MenuButtonControl`, `TabActions` |
| [[ui-engine-stack]] | **PARTIAL** — the UI is rebuilt in a new namespace; the old stack was deleted 2026-09-15 (6d, see `ui-engine-plan` § 6d), the `Next` prefixes and `Core.UISystem` went the same day; `Control` is CPU, `VulkanControl` is one GPU quad. Landings 1–5 and 6a built: it draws, measures, arranges, takes a per-window slice, dispatches the pointer and the wheel, renders a styled paragraph as one control with a row per glyph, samples images, icons, masks and gradients, and builds a tree from XML. the drag gesture runs press to release against whatever it is over, but tells its claimant nothing; context menus are [[context-menus]] | `UIEngine`, `Control`, `ControlXml`, `ContainerControl`, `WindowRoot`, `TextRunControl`, `CaretControl`, `ArrangeData`, `ControlGeometry`, `VulkanControl`, `CornerRadii`, `PointerEvent`, `UIEngineModule` |
...
| [[active-glfw-window-context]] | **SUPERSEDED 2026-09-15** — the focus latch was deleted with the old stack at 6d, nothing read it; the drag-raise lives on in `UIEngine.SolveDragWindow`. Was: the active window is a GLFW focus latch; a drag raises what it hovers | `AGlfwWindow`, `RenderWindow`, `UICollisionHandling` |
...
| [[ecs-rework-data-pools]] | **PARTIAL** — OO components become pooled columns; handles, compaction; command lanes (deleted 2026-09-23); a typed column found by `ColumnKey<T>` index, not `Dictionary<Type>` (2026-09-26) | `DataPool`, `DataManager`, `PoolColumn`, `ColumnKey`, `TransformData` |
| [[pool-shrink]] | pools double when full, halve after 2 s at a quarter full; lowest free stableId first | `DataPool.TryShrink`, `Resize`, `UIEngineModule.MirrorTooBig` |
...
| Note | Settles | Key symbols |
|---|---|---|
| [[frame-scheduler]] | **PARTIAL** (steps 1–3 of `frame-scheduler-plan`; step 3 `Jobs.For` splits Animation's rows across workers, thread count from chunk count) — a frame is steps in `Frame.frame.xml`: `Action=` names a `[A_XSDActionDependency(_, "Frame")]` method of a system, `Edge=` a pool's frame edge; stages computed from per-column reads/writes, a loop is a load error; steps of one system never overlap; Main is 5 pinned steps, Physics runs after entity logic; command lanes deleted, no mailbox; epoch/`LastTickMs` once per frame; Render is `Dedicated`; `Threads`/`MaxFps` settings; scheduler overhead ~20 µs a frame (7 stages) | `FrameScheduler`, `FrameStep`, `ThreadingSettings`, `ThreadedSystem.RunStep`/`EndFrame`, `MainSystem`, `DataPool.AssertAccess`, `DataPool.ElementBytes` |
| [[animation-core]] | **PARTIAL** (slice 3 of `animation-plan`) — animation is a system stepped by the frame graph ([[frame-scheduler]]); Main writes track rows directly and wakes them onto an awake list, the only tracks `Animation.Step` visits (physics-style sleep, 2026-09-25); no Main step applies values (`Main.Apply` gone 2026-09-26) — layout drains `LayoutDirty`, the next `Main.Logic` drains `AnimationDone` for `onDone`; every animatable property is pool-stored (`[A_Animatable(column, field, LayoutChange)]`), written in place by `Animation.Step` — since 2026-09-25 alpha, edge, button state and menu `reveal` too, `visual` being a `UIElements` column and `PaintRow` deriving the drawn row; ids and bindings live on Main with a generation; springs follow named signals, woken when one changes; `*.anim.xml` clips are keyframe tracks over a `Keyframes` pool, held hover/press clips reverse by `Direct`, state bindings are one mapped spring per bound property; no start/value ceilings since 2026-09-23; ~0.34 µs a track, `StopAll` O(bindings); chunks decide each track's outcome and `Emit` only stitches them; a track is a 64-B hot row plus per-driver params columns (2026-09-26) | `AnimationSystem`, `AnimationTrack`, `TweenParams`, `KeyParams`, `SpringParams`, `TrackCold`, `Animations` (`Play`, `Direct`, `Wake`, `Awake`, `DrainDone`), `AnimationLibrary`, `StateBinding`, `Signals`, `Curve`, `Spring`, `A_Animatable`, `AnimatableProperty`, `DirtyLayout`, `FinishedTrack`, `UIEngine.DrainLayoutDirty`, `Control.PaintRow` |
...
| [[entity-tick-group]] | ticking is explicit — `SetTicking` puts an entity on a packed tick list (slot on the entity, O(1) swap-remove); override detection only filters the start/enable queues; `"Entities"` list deleted; List/HashSet/array measurements | `Entity.SetTicking`, `tickSlot`, `HooksOf`, `EntityRegistry.ProcessTicks`/`SetTicking` |
...
| Note | Settles | Key symbols |
|---|---|---|
| [[render-window-owns-the-swapchain]] | a window owns its swapchain, sync and modules; the renderer keeps only the device; surface formats and present modes are cached per window; a rebuild passes the old swapchain; a resize the swapchain already matches skips its rebuild, and settings requests have their own flag; modules do not wait on the acquire — one submit, two batches; compositor descriptors are written only at init and rebuild | `RenderWindow`, `Renderer`, `AGlfwWindow`, `AVulkanHelper.GetSurfaceFormats`, `Renderer.RecreateSwapchain`, `Renderer.SwapchainMatchesSurface`, `RenderWindow.rebuildRequested`, `Renderer.Draw`, `CompositorModule.UpdateModule` |
...
| [[mapped-streaming-buffers]] | per-frame buffers are mapped and per-image; staging is for upload-once data; never read back; the UI draw is indirect, recorded once per image, its mirrors sized to the window's range | `AVulkanBufferHandler`, `Renderer`, `MCUI`, `UIEngineModule.MirrorDrawList`, `UIEngineModule.HasPendingWork`, `AVulkanBufferHandler.WriteMappedRange` |
