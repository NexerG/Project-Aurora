...
| [[ui-engine-stack]] | **PARTIAL** — the UI is rebuilt in a new namespace beside the old one; `Control` is CPU, `VulkanControl` is one GPU quad. Landings 1–4 built: it draws, measures, arranges, takes a per-window slice, dispatches the pointer, and renders a styled paragraph as one control with a row per glyph | `UIEngine`, `Control`, `WindowRoot`, `TextRunControl`, `NextCaretControl`, `ArrangeData`, `ControlGeometry`, `VulkanControl`, `PointerEvent`, `UIEngineModule` |
