...
namespace ArctisAurora.Core.UI
{
    ...
    public class NextSplitViewControl : NextStackPanelControl
    {
        ...
        // the other.
        public static NextTabViewControl Split(NextTabViewControl source, SplitEdge edge)
        {
            if (source.parent is not Control host) return null;
            ...
            float half = MathF.Max(paneMinimum, (main - gripThickness) * 0.5f);

            NextSplitViewControl split = new NextSplitViewControl
            {
            ...
            NextTabViewControl fresh = NewPane(source);
            Control first = sourceFirst ? source : fresh;
            Control second = sourceFirst ? fresh : source;
        ...
        public static void Collapse(NextSplitViewControl split, Control leaving)
        {
            if (split.parent is not Control host) return;

            Control survivor = null;
            foreach (Entity e in split.children)
                if (e is Control pane && pane is not NextSplitterControl && !ReferenceEquals(pane, leaving))
                {
                    survivor = pane;
                    break;
                }
        ...
        private static NextTabViewControl NewPane(NextTabViewControl source)
        {
            NextTabViewControl pane = source.NewOfSameKind();
        ...
        private static NextSplitterControl NewGrip(bool vertical, NextTabViewControl source)
        {
            NextSplitterControl grip = new NextSplitterControl
            {
        ...
        // shape NextSplitterControl resizes.
        private static void SizePane(Control pane, bool vertical, float fixedMain)
        {
            pane.widthStar = 0f;
            pane.heightStar = 0f;
